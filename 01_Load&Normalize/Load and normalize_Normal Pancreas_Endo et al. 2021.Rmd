...
```{r}
baseDir <- "T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/00_Methylation Data/2021_Endo_PDAC, Normal Pancreas/Idat Files_Normal Pancreas"
...
## to save plots:
```{r}
pdf("T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/Quality Control/P-values_NP_Endo.pdf")
barplot(colMeans(Pvalue), col=pal[factor(targets$Sample_Group)], las=2, 
        cex.names=0.8, ylab="Mean detection p-values")
abline(h=0.05,col="red")
legend("topleft", legend=levels(factor(targets$Sample_Group)), fill=pal,
...
## To save plot
```{r}
pdf("T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/Quality Control/BadSamples_NP_Endo.pdf")
qc <- getQC(MSet)
MSetEx <- addQC(MSet, qc = qc)
plotQC(qc)
dev.off()
...
qcReport(RGSet, 
         ...
         pdf= "T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/Quality Control/QC-report_NP_Endo.pdf")
```
### Sex prediction
```{r}
...
betas.ssnoob.NPEndo <- getBeta(MSet.ssnoob)
```

## to save data
```{r}
save(MSet.ssnoob, file = 'T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/PreProcessedDataSets/nonfilteredMSet.noobNPEndo14032022.rData')

save(betas.ssnoob.NPEndo, file = 'T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/PreProcessedDataSets/nonfilteredbetas.noobNPEndo14032022.rData')

save(betas.ssnoob.NPEndo, file = 'T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/PreProcessedDataSets/nonfilteredbetas.noobNPEndo14032022.rds')
```

# effect of normalisation:
```{r}
par(mfrow = c(2, 2))
pdf("T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/Quality Control/EffectNormalization_NP_Endo.pdf.pdf")
...
par(mfrow=c(1,2))
pdf("T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/Quality Control/Normalization_NP_Endo.pdf.pdf")
...
save(bValsnoobdf, file = 'T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/01_Load&Normalize/PreProcessedDataSets/preprocessedNoobNPEndo14032022.rData')
```
