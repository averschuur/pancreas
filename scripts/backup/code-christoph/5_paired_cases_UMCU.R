...
anno_umcu <- read_csv(file = "./input/annotation/cleaned/annotation_umcu.csv")
anno_paired <- read_csv(file = "./input/annotation/cleaned/annotation_umcu_paired_samples.csv")

idats <- detect_idats(dir = "./input/idat/")
idats <- idats[match(anno_umcu$array_id, idats$array_id), ]
...
# extract beta values
betas <- minfi::preprocessNoob(raw) %>% 
  getBeta()
rm(raw)
