...
anno %>% 
  ...
  paletteer::scale_fill_paletteer_d("rcartocolor::Safe", labels = c("Normal", "Tumor")) +
...
ggsave("Figure 2A_count Dataset by Location.pdf", path= "./plots/", dpi=500)
...
perf_acc %>%  
  ...
  ylim(0, 1)
ggsave("Figure2_Algorithm Performance.pdf", path= "./plots/", dpi=500)
...
perf_per_class %>% 
  ...
  labs(title="Algorithm Performance",x = NULL, y = "Accuracy (test cohort)") +
  ylim(0, 1)
ggsave("Figure1_model_comparison_accuracies_randforest.pdf", path= "./plots/", dpi=500)
...
perf_per_class2 %>% 
  gather(key = algorithm, value = Value, c("Accuracy_rf", "Accuracy_xgb","Accuracy_nn")) %>%
  ggplot(aes(class, Value, fill = algorithm)) + 
  geom_col(position = "dodge") +
  paletteer::scale_fill_paletteer_d("rcartocolor::Safe") +
  scale_x_discrete(limits = c("ACC", "NORM", "PanNEC", "PanNET","PB", "PDAC", "SPN"),
  ...
  theme(plot.title = element_text(face = "bold", hjust = 0.5, size = 12),
        ...
        panel.grid = element_blank()) +
  ylim(0, 1)
ggsave("Figure1_model_comparison_accuracies_all models.pdf", path= "./plots/", dpi=500)
...
ggsave("Figure2_correct classification.pdf", path= "./plots/", dpi = 500)
