...
anno_tcga <- read_csv("./annotation/annotation_tcga_subsampled.csv")
...
tcga_labels <- c("ACC", "Normal", "PanNEC", "PanNET", "PDAC", "SPN")
...
anno_tcga %>% 
  ggplot(aes(nn_max)) +
  geom_histogram(fill = branded_colors[1], alpha = 0.5, col =  branded_colors[1]) +
  theme_classic(base_size = 24) +
  labs(x = "Highest Score", y = "Count")

anno_tcga %>% 
  ggplot(aes(tissue, nn_max)) +
  geom_jitter(width = 0.2, col = branded_colors[1]) +
  theme_classic(base_size = 24) +
...
superheat::superheat(test2, 
                     ...
                     heat.pal = c("white",  branded_colors[7]), 
