...
ggplot(tsne_plot)+ 
  ...
  scale_color_npg(labels = c("ACC", "panNET", "SPN"))+
  theme(
...
row.names(pd) <- colnames(tMVP)
names(pd)[1]<-"Type"
pd[,1]<-as.character(pd[,1])

#create heatmap
MVPhm <- MVPanno
rownames(MVPhm) <- MVPhm[,6]
MVPhm[,1:6] <- NULL
MVPhm <- as.data.frame(t(MVPhm))

a<-cor(MVPhm, method = "pearson", use = "everything")
heatmap(a)
heat <- heatmap(a)
save_pheatmap_pdf(heat, "T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/results/UHC_using pearson1.pdf")
