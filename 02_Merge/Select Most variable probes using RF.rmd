...
#merge dataframes within list with sample group annotation
...
names <- rownames(rf2)
rfselection1 <-cbind(names,rf2)

rfselection <- as.data.frame(rfselection1[order(rfselection1$MeanDecreaseGini, decreasing = TRUE),])
rfselection <- as.data.frame(rfselection[1:5000,])
```

# make new df of the MVPs and redo RF
```{r}
# create df with most important MDG's 
pre <- merge(rfselection, merged, by.x="names", by.y="row.names")
rownames(pre) <- pre[,1]
pre[,1:2] <- NULL
...
#Save MVPs
save(tpre, file = 'T:/pathologie/PRL/Groep-Brosens/2. Anna Vera/2. ACN-SPN-NET/Pancreas-ID/04_RandomForest/5000MVPbyRF.Rdata')
